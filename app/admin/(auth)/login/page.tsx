...
import { setAuthToken, clearAuthToken, authFetch } from '@/lib/session-client';
...
function AdminLoginForm() {
  ...
  async function routeAfterLogin(token: string) {
    ...
    if (!me?.user?.staffRole) {
      clearAuthToken(); // this account has no staff access — don't leave a session token behind
      setError('This account has no Admin Panel access.');
      return;
    }
  ...
  const TITLES: Record<Step, { title: string; subtitle: string }> = {
    login: { title: 'Admin sign in', subtitle: 'Staff access only.' },
    ...
    <div className="mx-auto flex max-w-sm flex-col gap-6 py-14">
      <div className="flex flex-col items-center gap-3 text-center">
        <span className="flex h-14 w-14 items-center justify-center rounded-full bg-navy/5">
          <ShieldCheck className="h-6 w-6 text-navy" strokeWidth={1.75} />
        </span>
        <div>
      ...
      {step === 'login' && (
        <p className="text-center font-body text-xs text-ink-soft">
          Staff accounts are granted by a super admin, not self-registered — ask the Idara team if
          you need access.
        </p>
      )}
    </div>
